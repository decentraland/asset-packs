...
function main() {
  ...
  const packs = fs.readdirSync(ROOT).map((p) => path.join(ROOT, p)).filter(isDirectory);
  for (const packDir of packs) {
    const assetsDir = path.join(packDir, 'assets');
    if (!isDirectory(assetsDir)) continue;
    const items = fs.readdirSync(assetsDir).map((p) => path.join(assetsDir, p)).filter(isDirectory);
